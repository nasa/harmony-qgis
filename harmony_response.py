...
def download_image(response, layerName):
    ...
    directory = settings.value(
        "harmony_qgis/download_dir") or tempfile.gettempdir()
        ...
        if result is None:
            ...
            for layerName, fileName in new_layers:
                ...
                if not layer or not layer.isValid():
                    iface.messageBar().pushMessage("Error", "Failed to create layer {}".format(
                        layerName), level=Qgis.Critical, duration=0)

            if status != 'done':
                response = result['response']
                globals()['task'] = QgsTask.fromFunction('Worker', pollResults,
                                                         on_finished=completed, iface=iface, response=response, link_count=link_count)
                QgsApplication.taskManager().addTask(globals()['task'])
            else:
                status = "Download complete - {} new layers created".format(
                    link_count)
                iface.mainWindow().statusBar().showMessage(status)
    else:
        iface.messageBar().pushMessage("Error", 'Error while processing downloads',
...
def handleAsyncResponse(iface, response, background):
    ...
    if background:
        globals()['task'] = QgsTask.fromFunction('Worker', pollResults,
                                                 on_finished=completed, iface=iface, response=response, link_count=0)
        QgsApplication.taskManager().addTask(globals()['task'])
    else:
        result = pollResults(None, iface, response, 0)
        while result['status'] != 'done':
            result = pollResults(
                None, iface, result['response'], result['link_count'])
        response = result['response']
...
def handleSyncResponse(iface, response, layerName, variable):
    ...
    iface.addRasterLayer(tempfile.gettempdir() + os.path.sep +
                         'harmony_output_image.tif', layerName + '-' + variable)
