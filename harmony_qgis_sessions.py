...
def startDeleteSession(mainDlg, dlg):
    ...
    if dlg.listWidget.currentItem():
        ...
        tempSavedSessions = list(
            filter(lambda session: session[0] != sessionName, savedSessions))
...
def importSessions(mainDlg, dlg, fileName=None):
    if not fileName:
        fileName, _filter = QFileDialog.getOpenFileName(
            dlg, 'Open sessions file', '', "Json files (*.json)")
    if fileName:
        newSessions = []
        try:
            ...
            QMessageBox.critical(
                None, "Error!", "Failed to parse {}".format(fileName))
    ...
    if len(exportedSessions) > 0:
        ...
        if fileName:
            ...
            try:
                ...
                QMessageBox.critical(
                    None, "Error!", "Failed to export session data to {}".format(fileName))
